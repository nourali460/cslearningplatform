    ...
    try {
      ...
      const payload = isAdmin
        ? {
            ...
            defaultMaxPoints: parseFloat(formData.maxPoints) || 0,
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to create lesson template')
      }
      ...
      console.error('Error saving lesson template:', err)
      setError(err instanceof Error ? err.message : 'Failed to save lesson template')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-3xl font-bold flex items-center gap-2">
            <BookOpenCheck className="h-8 w-8 text-accent-purple" />
            📖 {editId ? 'Edit' : 'Create'} Interactive Lesson Template
          </h1>
        ...
        <p className="text-muted-foreground">
          {isAdmin
            ? 'Create a reusable interactive lesson template for a course. This template can be used by all professors teaching this course.'
            : 'Create an interactive lesson for your class with rich multimedia content and engaging learning experiences.'}
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isAdmin
              ? 'Define the lesson template settings. Professors can customize these when creating actual lessons.'
              : 'Configure the interactive lesson for your students.'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Describe the lesson learning objectives, topics covered, and what students will learn..."
              />
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="maxPoints">
                Maximum Points <span className="text-muted-foreground text-xs">(Optional - leave at 0 for non-graded lessons)</span>
              </Label>
              <Input
                id="maxPoints"
                type="number"
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="NONE">No Submission (View Only)</SelectItem>
            ...
            {!isAdmin && (
              <div className="space-y-2">
                <Label htmlFor="dueAt">
                  Completion Deadline <span className="text-muted-foreground text-xs">(Optional)</span>
                </Label>
                <Input
                  id="dueAt"
                  type="datetime-local"
            ...
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="includeInGradebook" className="cursor-pointer">
                Include in gradebook <span className="text-muted-foreground text-xs">(only if graded)</span>
              </Label>
            </div>
      ...
      <Card className="border-dashed">
        <CardHeader>
          <CardTitle className="text-lg">Planned Interactive Lesson Features</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-muted-foreground">
            <div>• Drag-and-drop content builder</div>
            <div>• Rich media embedding (videos, images, audio)</div>
            <div>• Live code demonstrations with execution</div>
            <div>• Embedded knowledge checks and quizzes</div>
            <div>• Progress tracking and completion analytics</div>
            <div>• Interactive diagrams and simulations</div>
          </div>
        </CardContent>
      </Card>
    </div>
