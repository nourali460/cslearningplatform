  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to create lab template')
      }
      ...
      console.error('Error saving lab template:', err)
      setError(err instanceof Error ? err.message : 'Failed to save lab template')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-3xl font-bold flex items-center gap-2">
            ...
            🧪 {editId ? 'Edit' : 'Create'} Lab Template
          </h1>
        ...
        <p className="text-muted-foreground">
          {isAdmin
            ? 'Create a reusable lab template for a course. This template can be used by all professors teaching this course.'
            : 'Create a lab assignment for your class. Students will submit code or files for grading.'}
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isAdmin
              ? 'Define the lab template settings. Professors can customize these when creating actual assignments.'
              : 'Configure the lab assignment for your students.'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="title">
                Lab Title <span className="text-error">*</span>
              </Label>
              <Input
                id="title"
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Describe the lab objectives, requirements, and instructions..."
              />
            </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t">
              <Button type="submit" disabled={saving}>
                ...
                {saving ? 'Saving...' : editId ? 'Save Changes' : 'Create Lab'}
              </Button>
              <Button
                type="button"
                variant="outline"
      ...
      <Card className="border-dashed">
        <CardHeader>
          <CardTitle className="text-lg">Planned Lab Features</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-muted-foreground">
            <div>• Cloud IDE integration for in-browser coding</div>
            <div>• Starter code templates</div>
            <div>• Automated test cases</div>
            <div>• Auto-grading based on test results</div>
            <div>• Code review tools with inline comments</div>
            <div>• Multi-language support (Python, Java, C++, etc.)</div>
          </div>
        </CardContent>
      </Card>
    </div>
